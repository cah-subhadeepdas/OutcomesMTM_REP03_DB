...
--Description:- Loading the data Into Stage table. TC-3349

 
CREATE   procedure [reporting].[I_Patient_And_TIP_Trending_Report] 
AS
BEGIN


DECLARE @ReportRundate DATE
SET @ReportRundate=CAST(GETDATE() AS DATE)

/*Getting All Active patients for YTD*/--4089936
...
Drop table If exists #Patient
Select distinct p.PatientID,p.ClientID,p.policyID,p.CMREligible
Into #Patient
From AOCWPAPSQL02.outcomes.dbo.patient p With(NOLOCK)
Where 1=1
and Year(p.OutcomesEligibilityDate) =year(@ReportRundate)
and p.OutcomesTermDate>=@ReportRundate
...
and Year(tp.activeasof)=Year(@ReportRundate)
...
,PolicyID
,Case when activethru>@ReportRundate then 0 else withdrawn END AS withdrawn1
...
and Year(pot.createDT)=YEAR(@ReportRundate)
...
,T7.[Count Of CMR Eligible patients]
...
JOIN dbo.ClientContractPolicyView cv ON cv.policyID=po.policyID AND cv.activeflag=1
Left Join 
(
...
LEFT JOIN(
 ...
 Select Count(*) as [Count Of CMR Eligible patients],p.PolicyID
    From #Patient p with(nolock)
	WHERE 1=1
	AND p.CMREligible=1
    Group by p.PolicyID
)T7 ON T7.PolicyID=po.policyID
...
--Select * From #Final
--Select * From Staging.Patient_TIPTrending
/*Insert Into Actual table*/

Truncate table Staging.Patient_TIPTrending
Insert Into OutcomesMTM.Staging.Patient_TIPTrending
(
    [PolicyID] 
	,[ClientID] 
	,[PolicyName] 
...
[PolicyID]
	  ...
	  ,ISNULL([Count Of CMR Eligible patients],0)         As [Count Of CMR Eligible patients]
...
--Exec [reporting].[I_Patient_And_TIP_Trending_Report]
