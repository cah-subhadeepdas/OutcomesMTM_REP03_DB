...
CREATE    proc [dbo].[S_ReportCard_Pharmacy_TC2348] 
as 
begin 
...
---------------------------------------------------------------------------------------------------------------
If object_id('tempdb..#TEMP_Users') is not null
drop table #TEMP_Users
 SELECT distinct u.userID,(c.lastNM + ','+ c.firstNM) AS [UserName] 
 into #TEMP_Users
 from OutcomesMTM.staging.users u
 join OutcomesMTM.staging.contact c on c.userID = u.userID
 where (c.lastNM + ','+ c.firstNM) 
 IN(
 'Arias,Farah','Becks,Howard','Breslow,Wendy','Cadette,Rosie','Colosimo,Maria','Dowe,Yvette','Enriquez,Cristina','Fournier,Michael','Gonzalez,Avelino','IrizarryHernandez,Wilmiris',
'Irizarry,Jose','Jacobs,John','Leon,Miday','Louis,Donna','Marx,Frank','Miglio,Elio','Mitchell,Adexis','Pardo,Lynda','Patel,Kajal','Ruiz,Jessica','Salvatico,Gerald',
'Stone,Elizabeth','Varela,Cristina','Walker,Takenya','AbrahanteEspinosa,Ileana','Allen,Naila','Awadallah,Mary','Brewley,Kayla','Caceres,Laura','Daoud,Christine',
'Demesmin,Mirlotte','Desauguste,Ruth','Francois-Ottinot,Margarita','Gordo,Yvette','Hermida,Gissel','LeeSrebro,Jesse','Lue,Peter','McLaren,Yanique','McLin,Keysa',
'Medina,Camille','Menendez-Perez,Lidice','Modi,Pritesh','Mosquera,Olaya','Ortheus,Magalie','Pata,Daphney','Petion,Vana','Piedra,Onelio','Reed,Makita','Rossin,Tierra',
'Rutledge,Adaisha','Sager,Andra','Savage,Funmilola','Serrate,Liuva','Shaw,Patricia','Tam,Victoria','Urieto,Christopher','Vargas,Yeny','Wallace,Jonell','Wilson,Narges',
'Cook,John','Jose,Jimmy','RiveraSantiago,Adamaris'
)  

 
If object_id('tempdb..#TEMP_Claims') is not null
drop table #TEMP_Claims 
 SELECT distinct cl.claimID, u.userID, u.UserName
 into #TEMP_Claims 
 from claim cl join #TEMP_Users u on cl.pharmacistID = u.userID

 CREATE CLUSTERED INDEX TX_001 ON #TEMP_Claims (ClaimID)

--If object_id('tempdb..#TEMP_Exclude') is not null
--drop table #TEMP_Exclude
-- SELECT distinct t.claimID, t.userID, t.UserName, ch.chaincode
-- into #TEMP_Exclude 
-- from #TEMP_Claims t 
-- join OutcomesMTM.staging.userCenter uc on uc.userID = t.userID
...
-- join OutcomesMTM.dbo.chain ch on ch.chainid = pc.chainid
 --WHERE ch.chaincode in ('A10', 'A13' , '226')

 --CREATE CLUSTERED INDEX TX_002 ON #TEMP_Exclude (ClaimID)
   ------------------------------------------------------------------------------------------------------------------------------------------
  select * from ( 
...
--join OutcomesMTM.staging.userCenter uc on uc.userID = ph.centerid
--join OutcomesMTM.dbo.pharmacychain pc on pc.centerid = ph.centerid
--join OutcomesMTM.dbo.chain ch on ch.chainid = pc.chainid
-- WHERE ch.chaincode in ('A10', 'A13' , '226')
...
left join (
	...
	from (
		...
		and c.claimID not in (SELECT claimID from #TEMP_Claims) --Claims to be excluded
...
left join (
	...
	from (
        ...
        , case when rp.outcomesTermDate between @BEGIN and @END AND isNull(rp.mtmServiceDT, '99991231') not between @BEGIN and @END THEN 1 ELSE 0 END as Termed
			...
			and rp.claimID not in (SELECT claimID from #TEMP_Claims)  --Claims to be excluded
...
where 1=1
and np.Active = 1 
) rpt where rpt.Relationship_ID in ('A10','A13','226')
order by rpt.centerID
