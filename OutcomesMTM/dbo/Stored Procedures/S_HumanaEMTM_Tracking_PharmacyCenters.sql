...
CREATE PROCEDURE [dbo].[S_HumanaEMTM_Tracking_PharmacyCenters]
AS
BEGIN
...
If OBJECT_ID('tempdb..#EMTM_CMR') is not null
Drop table #EMTM_CMR
Select DISTINCT
		r.Region
...
--=============================================
--Get Centers with TIP Opportunities and Completed TIPsDrop table #GetDenominator
...
Select t.Region, t.OpportunityTIP as [EMTM Denominator], t.CompletedTIP as [EMTM Numerator]
Into #Tips_CMRs
From #Tips t
Union all
...
--Final Query
Select * from #DistinctTipsCMRs C ORDER BY C.Region
