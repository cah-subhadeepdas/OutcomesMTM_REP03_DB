...
Create   proc [dbo].[S_ReportCard_Client] 
as 
begin 
...
SET @BEGIN = CAST(case when month(getdate()) = 1 then cast(year(getdate()) - 1 as varchar(4)) + '-01-01' else cast(year(getdate()) as varchar(4)) + '-01-01' end AS DATE) --// Beginning of year (if current month is Jan. then beginning of last year)		
...
-- TC-3483 added by Li
drop table if exists #VirtualsOnly
select	DISTINCT [Organization Name]
		, ncpdp_nabp
into	#VirtualsOnly 
from	#Org2Center
where	NCPDP_NABP like '99%'
create clustered index IDX on #VirtualsOnly (ncpdp_nabp)

-- TC-3483 added by Li
-- Get primary pharmacy NCPDP_NABP for all patients
DROP TABLE IF EXISTS #PatientprimaryPharmacy_OrgName
SELECT	*
INTO	#PatientprimaryPharmacy_OrgName
FROM	
(
	...
	FROM	AOCWPAPSQL02.outcomes.dbo.patient pt
			...
			JOIN AOCWPAPSQL02.outcomes.dbo.pharmacy ph ON ph.centerid = ppp.centerid
			JOIN #Org2Center org ON org.NCPDP_NABP = ph.NCPDP_NABP
	WHERE	ppp.primaryPharmacy = 1
)	Orgname
CREATE CLUSTERED INDEX IDX ON #PatientprimaryPharmacy_OrgName ([Organization Name],PatientID)
...
-- TIP section
-- Get all patient's TIPs records from primary pharmacy include NCPDP and orgName
-- TC-3483 added by Li
...
-- TC-3483 added by Li
-- Get all patient's TIPs records from virtual pharmacy
DROP table if exists #TIP_Virtual_primaryPharmacy
select	* 
into	#TIP_Virtual_primaryPharmacy
from
	(
		SELECT	row_number() over (partition by oc.orgID, tacr.tipresultstatusID order by tacr.[Completed TIPs] desc, tacr.[Unfinished TIPs] desc, tacr.[Review/resubmit Tips] desc, tacr.[Rejected Tips] desc, tacr.[currently active] desc, tacr.[withdrawn] desc, tacr.tipresultstatuscenterID desc) as [Rank] 
				, tacr.centerid
		...
		FROM	outcomesMTM.dbo.tipActivityCenterReport tacr with(nolock)
				join #org2Center oc on oc.centerid = tacr.centerID
				join #VirtualsOnly v on v.ncpdp_nabp = tacr.ncpdp_nabp
				JOIN #PatientprimaryPharmacy_OrgName po ON po.[patientid] = tacr.[patientid] AND po.[Organization Name] = v.[Organization Name]
				join 
					(
						SELECT	po.policyID
								, cl.clientID
...
-- Merge real pharmacy data with virtual pharmacy data
-- TC-3483 added by Li
...
FROM
	(
		...
		UNION ALL
		SELECT	*
		FROM	#TIP_Virtual_primaryPharmacy t 
	)	FinalTIP
...
-----------------------------------------------------------------------------------------------------------------
-- CMR section
-- Get all patient's CMR records from primary pharmacy
...
FROM
	(
		SELECT	row_number() over (partition by rp.patientID, oc.orgID order by isNull(rp.resultTypeID, 90), rp.mtmserviceDT desc, rp.patientKey, rp.patientMTMCenterKey) as rank
				...
				, case when rp.mtmServiceDT between @BEGIN and @END AND rp.statusID in (2, 6) and rp.resultTypeID in (5, 6) AND ISNULL(rp.centerID,'') <> ISNULL(rp.claimcenterID,'') THEN 1 ELSE 0 END as CMRMissed
...
-- Get all patient's CMR records from virtual pharmacy 
DROP table if exists #CMR_Virtual_Pharmacy
select	* 
into	#CMR_Virtual_Pharmacy
from
	(
		SELECT	row_number() over (partition by rp.patientID, oc.orgID order by isNull(rp.resultTypeID, 90), rp.mtmserviceDT desc, rp.patientKey, rp.patientMTMCenterKey) as rank
				, 0 as [CMR Opportunities]
				, rp.PatientID
				, rp.PolicyID
				, rp.CMSContractNumber
				, oc.orgID
		...
		FROM	vw_CMRActivityReport rp with(nolock)
				...
				join #VirtualsOnly v on v.ncpdp_nabp = ph.ncpdp_nabp
		WHERE	1=1    
				AND isNull(rp.activethru, '99991231') >= @BEGIN
				AND rp.activeasof <= @END
					--30 day include
