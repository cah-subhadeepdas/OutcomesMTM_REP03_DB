...
CREATE   proc [dbo].[S_ReportCard_Organization] 
as 
begin 
...
-- Re-sign the value of [TIP Opportunities] from #TIP_Virtual_primaryPharmacy, buiness doesn't want to double the [TIP Opportunities]
update #TIP_Virtual_primaryPharmacy
set [TIP Opportunities] = 0
...
from
	(
		...
		from	vw_CMRActivityReport rp
				...
				JOIN #PatientprimaryPharmacy_OrgName po ON po.[patientid] = rp.[patientid] AND po.[Organization Name] = v.[Organization Name]
		where	1=1
				AND isNull(rp.activethru, '99991231') >= @BEGIN
				AND rp.activeasof <= @END
				--30 day include
...
from	#org o
		...
		left join 
				(
					...
					select	u.orgID				
							, cast(sum(u.[CMR Opportunities]) as decimal) as [CMROpportunity]
