...
CREATE PROCEDURE [dbo].[U_PRESCRIPTIONDIM]
      @queueOrder int 
AS 
BEGIN
...
BEGIN TRAN
   ...
   SET NOCOUNT ON;
   SET XACT_ABORT ON;
  -- declare @queueOrder int = 1
   DECLARE @changeDate DATETIME = GETDATE(); 
   DECLARE @recordCount BIGINT = 0

   ----------------------------TEMP TABLES FOR AUDIT--------------------------------	

	Drop table if exists #prescriptionDim_Audit
	CREATE TABLE #prescriptionDim_Audit(	
		...
		SourceQueueDate datetime null, 
		SourceQueueOrder int null
		)
...
-----------------------------------------------------
   ...
   UPDATE d  
	  ...
	  OUTPUT inserted.RxID , inserted.patientid , inserted.NCPDP_NABP , inserted.RxNumber , inserted.RxDate , inserted.NDC , inserted.activeAsOf, inserted.activeThru ,
			 1 as isInsert , 1 as isDelete , 'Update' as OperationType , inserted.activeThru as SourceQueueDate , @queueOrder as SourceQueueOrder into #prescriptionDim_Audit
   --SELECT count(*) 
   FROM prescriptionDim d --WITH (INDEX (UK_prescriptionDim_activethru))
      JOIN prescriptionDeltaQueueStaging s 
        ...
        AND d.[RxDate] = s.[RxDate] -- TC-3350..Removed to reflect the proper composite key after PrescriptionRefactor 
        AND d.[NDC] = s.[NDC]   -- TC-3350..Removed to reflect the proper composite key after PrescriptionRefactor 
 ...
 -- Disabling the pescriptiondim indexes
   ...
   INSERT INTO prescriptionDim (
	...
	OUTPUT   inserted.RxID , inserted.patientid , inserted.NCPDP_NABP , inserted.RxNumber , inserted.RxDate , inserted.NDC , inserted.activeAsOf, null as activeThru ,
			 1 as isInsert , 0 as isDelete , 'Insert' as OperationType , inserted.activeAsOf as SourceQueueDate , @queueOrder as SourceQueueOrder into #prescriptionDim_Audit 
	...
	-- AND s.isDelete = 0 --added this line for PrescriptionDim refactor
     AND NOT EXISTS( SELECT 1 
                     ...
                     WHERE 1=1 
                       ...
                       AND d1.[RxDate] = s.[RxDate]  --TC-3350..Removed to reflect the proper composite key after PrescriptionRefactor 
                       AND d1.[NDC] = s.[NDC]   --TC-3350..Removed to reflect the proper composite key after PrescriptionRefactor 
                       AND d1.activeasof = s.queueDate
					   AND d1.active = s.active
                    )
     AND NOT EXISTS( SELECT 1 
                     ...
                     WHERE 1=1 
                       ...
                       AND d2.[RxDate] = s.[RxDate] --TC-3350..Removed to reflect the proper composite key after PrescriptionRefactor 
                       AND d2.[NDC] = s.[NDC]  --TC-3350..Removed to reflect the proper composite key after PrescriptionRefactor 
                       AND d2.activethru IS NULL 
					   AND d2.active = s.active
                    )
...
-- Enabling the pescriptiondim indexes
 ...
 /* --Commenting Out Deletes after Prescription Refactor --No more scenarios of isDelete=1 , isInsert=1 --start
  ...
  INSERT INTO prescriptionDimDataLog (
   ...
   WHERE 1=1 
	  ...
	  */ --Commenting Out Deletes after Prescription Refactor --No more scenarios of isDelete=1 , isInsert=1 --end
   -----------------------------------------------------

   /*--Commenting Out for code-optimization-start
   --Re-initialize @recordCount
   SET @recordCount = 0
  ...
  --Create New Record In Logging Table For Start of #tempRepository INSERTS
   ...
   WHERE 1=1 
     ...
     AND enddate IS NULL
	*/--Commenting Out for code-optimization-end


  ------ Audit Activity ----
    create nonclustered index NC_prescriptionDim_Audit on #prescriptionDim_Audit (patientid , NCPDP_NABP , RxNumber , RxDate , NDC , SourceQueueOrder , OperationType)

	INSERT INTO [dbo].[prescriptionDeltaQueueNotPublished]
           ([RxID]
           ,[patientid]
           ,[NCPDP_NABP]
           ,[RxNumber]
		...
		WHERE 1=1 
			 ...
			 AND NOT EXISTS( SELECT 1 
							 ...
							 WHERE 1=1 
							   ...
							   AND d1.[NDC] = s.[NDC] --TC-3350..Removed to reflect the proper composite key after PrescriptionRefactor 
							   --AND d1.activeasof = s.queueDate
							   AND d1.SourceQueueOrder = @queueOrder
							   and d1.OperationType in ('Insert', 'Update')
							)
			 AND NOT EXISTS( SELECT 1 
							 FROM prescriptionDim d2 
							 WHERE 1=1 
							   AND d2.[patientid] = s.[patientid]
							   AND d2.[NCPDP_NABP] = s.[NCPDP_NABP]	
							   AND d2.[RxNumber] = s.[RxNumber]
