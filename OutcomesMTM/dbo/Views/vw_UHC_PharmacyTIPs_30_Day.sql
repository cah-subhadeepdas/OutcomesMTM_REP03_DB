...
CREATE view [dbo].[vw_UHC_PharmacyTIPs_30_Day]
as 
...
, t1.[Count of Opportunities > 30 days]
...
(
	...
	, ISNULL(cast((cast(sum(t.[Completed TIPs]) as decimal)/NULLIF(cast(sum(t.[30dayTip]) as decimal), 0)) as decimal (5,2)), 0) as [% Completed]
	...
	, ISNULL(cast((cast(sum(t.[Successful TIPs]) as decimal)/NULLIF(cast(sum(t.[30dayTip]) as decimal), 0)) as decimal (5,2)), 0) as [% Net Effective]
	from (
