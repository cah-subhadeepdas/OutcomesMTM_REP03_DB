...
CREATE   PROCEDURE [cms].[U_Beneficiary_MTMPEnrollment__bak_20191223]
AS
...
BEGIN
	...
	declare @CYFromDate date =   (cast(year(getdate()) as varchar(4)) + '-01-01')
	declare @CYThruDate date =  (cast(year(getdate()) as varchar(4)) + '-12-31')
	declare @LYThruDate date =  (cast(year(getdate()) -1 as varchar(4)) + '-12-31')
...
BEGIN TRY
	...
	BEGIN TRANSACTION
		...
		DECLARE @errDupeFile varchar(8000)
		select @ErrDupeFile = 'FileID: ' + cast(f.FileID as varchar) + ' | FileNM: ' + isnull(f.FileNM,'')
		from #FileLoad_stg stg
		join cmsETL.FileLoad f
			on f.FileNM = stg.FileNM
			and f.LoadDT = stg.LoadDT
			and f.FileTypeID = 1
			and f.ActiveThruDT > getdate()
		where 1=1


		IF (@errDupeFile is not null)
		BEGIN
			SET @return = '[CMS_Process_Exception] U_Beneficiary_MTMPEnrollment: Active File has aleady been processed: ' + @errDupeFile;
	...
	--// debugging:  insert into #IngestLog__INSERTED select il.* from cmsETL.BeneficiarySF_IngestLog il where il.FileID = 1153
	...
	--// debugging:  declare @FileID int = 1153
	...
	--// debugging:  declare @ValidationDataSet int = 2, @BatchKey varchar(255) = 'FileID', @FileID varchar(255) = '1153'
	...
	BEGIN
		...
		----	, PatientID_All = m.PatientID_All
		----	, AuditActivityTypeID = 1
		----	, AuditDetailAttributeName = 'MI'
		----	, AuditDetailAttributeNameExternal = 'Middle_Initial'
		----	, AuditDetailValue_OLD = bu.MI__OLD
		----	, AuditDetailValue_NEW = bu.MI
		----from #Beneficiary_stg__UPDATED bu
		----join #MTMPEnrollment_stg m
		----	on m.PatientID = bu.PatientID
		----where 1=1
		----and isnull(bu.MI,'') <> isnull(bu.MI__OLD,'')
